...
- **Models**: `gemini-2.0-flash-exp` (Gemini Flash) for fact extraction and content quality assessment
- **Tools**: DataForSEO MCP (backlinks_domain_summary) for domain authority with 7-day caching
- **Trigger**: Runs automatically after Phase 1 completes if `elite_competitors` list exists
- **Logic**: Validates credibility of each Exa.ai source using multi-factor scoring algorithm (0-100 scale). Each source evaluated against 5 factors: (1) Domain Authority from DataForSEO backlinks API (40 pts max, DA × 0.4), (2) Domain Type categorization .gov/.edu (20pts), research journals (15pts), major publishers like NYT/WSJ/HBR (10pts), (3) Content Freshness via publish date extraction <1yr (15pts), <2yr (10pts), <3yr (5pts), (4) Internal Citations count of credible backlinks found within source content (15 pts max, 5pts per credible citation), (5) Content Quality via Gemini Flash assessment (10 pts max). **Minimum threshold: 60/100 to pass verification**.
...
- **Fact Extraction**: Uses Gemini Flash to extract verifiable factual claims from each verified source (credibility ≥60). Extracts 4 fact types: (1) statistics (percentages, numbers, dollar amounts), (2) benchmarks (industry standards, thresholds), (3) case_studies (real-world outcomes, company examples), (4) expert_quotes (direct quotes from named experts). Returns JSON array with fact_text, fact_type, citation_anchor, and confidence (0.0-1.0). Only stores facts with confidence ≥0.7. Cost: ~$0.0001 per source.
- **Citation Map Building**: Stores extracted facts in FactCitation table linked to VerifiedSource. Citation map passed to WriterService with format: `{fact_text: {url, title, anchor, confidence}}`. Injected into Claude's prompt for inline markdown citation generation.
...
- **Logic**: Operates in an **iterative triple-gate loop** (max 5 attempts). Each draft is validated by three sequential gates:
  ...
  2. **Gate 3 (Citations)**: `verify_citation_requirements` enforces minimum 3 inline markdown citations. Only runs if `research_run_id` provided (Phase 1.5 completed). Regex pattern `\[([^\]]+)\]\(https?://[^\)]+\)` counts citations. Triggers rewrite if <3 found with feedback: "Only X citations found, need 3 minimum. Add more inline citations using [Source Title](URL) format."
...
- **Citation Map Injection**: If `research_run_id` provided (Phase 1.5 completed), fetches `FactCitation` rows from database and builds citation map: `{fact_text: {url, title, anchor, confidence}}`. Injected into prompt with citation instructions: "You have access to X verified factual claims with sources. When you mention any statistic, benchmark, or data point, you MUST cite the source using inline markdown links: [Source Title](URL). Cite at least 3-4 sources throughout the article. ONLY use facts from the citation map above." Citation map typically contains 10-30 facts from 3-5 verified sources. Claude weaves citations naturally into sentences during generation.

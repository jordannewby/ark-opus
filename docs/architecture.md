...
- **Fallback / Safety**: Circuit breaker forces output at 5 iterations. Hallucinated tools return a simulated error to allow R1 to self-correct. When R1 doesn't extract sources via `exa_extract_full_text`, `_exa_elite_discovery()` fallback fires using the normalized `niche` variable (March 2026 bugfix: was passing undefined `niche_context`).

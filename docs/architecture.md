...
- **Config**: `EXA_RESEARCH_TIMEOUT=300`, `EXA_RESEARCH_MODEL="exa-research"`, `EXA_RESEARCH_ENABLED=True` in settings.py
- **Cost**: Single async API call per generation. Reported cost per-call via `cost_dollars` field.
...
- **Claim verification**: `MAX_EXA_FACT_CHECKS`, `MAX_LLM_VERIFICATIONS`, `MAX_CLAIM_RETRIES`
...
- **Phase 1.5 Graceful Degradation** (April 2026): Exa Research API failure (`ExaResearchError`, `asyncio.TimeoutError`) is non-fatal. Pipeline yields `phase1_5_warning` SSE event and continues with Phase 1 data only. Writer handles zero-citations gracefully.
...
Unit tests in `tests/` (85 tests across 4 core files, April 2026):

- **test_writer_gates.py** (21 tests) — `_sanitize_banned_words()` (case preservation, inflected forms, word boundaries, multiple replacements) + `verify_seo_score()` (word count, H1/H2 counts, list/table blocks, banned word detection, info gain density, code block exclusion)
- **test_claim_verification.py** (27 tests) — `extract_article_claims()` (quantitative/qualitative extraction, bare link filtering, substance checks) + `_normalize_url()` (www stripping, query param removal, domain lowercasing) + `_QUANT_CLAIM_PATTERN` regex (percentages, dollar amounts, multipliers)
- **test_readability.py** (16 tests) — `passes_readability()` (ARI/FK boundaries, sentence length, complex sentence ratio, distribution gate) + `analyze_readability()` (simple/complex text, keyword masking) + `verify_readability()` (dict output format)
- **test_citation_laundering.py** (21 tests) — `detect_citation_laundering()` (org-domain matching, subdomain support, case insensitivity, anchor text) + `detect_attribution_mismatches()` (batch processing, text truncation, mixed claim lists) + `extract_domain()` (www stripping, subdomain preservation)

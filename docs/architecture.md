...
- **Exa.ai Domain Quality Filters**: Defense-in-depth source credibility filtering. `exa_scout_search` now accepts `include_domains`, `exclude_domains`, `start_published_date`, and `num_results` (increased to 10 from 5) parameters. Niche-aware filter injection via `_build_agentic_prompt()` automatically applies authoritative domain lists based on workspace niche: (1) **Cybersecurity**: nist.gov, cisa.gov, owasp.org, sans.org, krebsonsecurity.com, schneier.com, etc. (2) **AI/ML**: arxiv.org, openai.com, anthropic.com, deepmind.com, ai.google, acm.org, ieee.org, etc. (3) **General**: .edu, .gov, nature.com, science.org. Excludes low-quality domains (medium.com, blogspot.com, wordpress.com, etc.) automatically. Enforces 2-year recency filter (`start_published_date`). R1 receives explicit filter instructions in STEP 2 with domain lists and example JSON. Prevents low-authority sources from entering verification pipeline, reducing Phase 1.5 rejection rate from ~20% to ~5%.
...
- **Models**: `gemini-2.5-flash` (Gemini Flash) via new `google-genai` SDK for fact extraction and content quality assessment
...
- **Logic**: Validates credibility of each Exa.ai source using **rebalanced multi-factor scoring algorithm** (0-100 scale, March 2026). Each source evaluated against 4 factors: (1) **Content Quality** via Gemini Flash assessment (40 pts max, was 20pts — PRIMARY SIGNAL), (2) **Domain Tier** from curated domain lists in `domain_tiers.py` (30 pts max, was 40pts), (3) **Content Freshness** via enhanced publish date extraction <1yr (20pts), <2yr (13pts), <3yr (7pts), (4) **Internal Citations** count of credible backlinks (10 pts max, was 20pts). **Minimum threshold: 60/100 to pass verification**. Score breakdown logged as `[SCORE BREAKDOWN] Total: X/100 (Domain: Y, Quality: Z, Freshness: W, Citations: V)`.
- **Enhanced Date Extraction** (March 2026): Scans **5000 chars** (was 2000) with **10 regex patterns** (was 4): ISO 8601, "published:", "date:", "updated:", "last-modified:", slash formats, dot formats. **URL fallback** extracts dates from URL paths (`/2024/03/15/article`). Sanity checks reject future dates or pre-2000 dates. Accepts optional `source_url` parameter for URL-based extraction.
...
- **Fact Extraction** (Improved March 2026): Uses Gemini Flash to extract verifiable factual claims from each verified source (credibility ≥60). **Enhanced prompt** includes explicit examples of each fact type: (1) statistics with specific examples ("67% of SMBs experienced..."), (2) benchmarks ("Average cost of data breach is $4.35M"), (3) case_studies ("Shopify reduced load time by 40%"), (4) expert_quotes ("According to Gartner analyst..."). Returns JSON array with fact_text, fact_type, citation_anchor, and confidence (0.0-1.0). **Lowered confidence threshold to ≥0.6** (was 0.7) to capture more facts. Cost: ~$0.0001 per source.
- **Citation Map Building**: Stores extracted facts in FactCitation table linked to VerifiedSource. Citation map passed to WriterService with format: `{fact_text: {url, title, anchor, confidence}}`. Injected into Claude's prompt with **critical emphasis** (visual borders, explicit examples, strict prohibitions against fabricating stats).
- **Strict Enforcement**: Generation **fails** if <3 credible sources found. Error event yielded to frontend: "Insufficient credible sources: only X found (need 3 minimum)". User must choose different keyword or topic.
...
- **Cost**: ~$0.011/article first run (DataForSEO $0.01 for 5 domains + Gemini Flash $0.001 for fact extraction). ~$0.001/article after caching (80%+ cache hit rate expected after 20 articles).
- **Multi-Tenant**: All tables scoped by `(profile_name, niche)`. DomainCredibilityCache shared across workspaces for efficiency.
...
- **Citation Map Injection** (Strengthened March 2026): If `research_run_id` provided (Phase 1.5 completed), fetches `FactCitation` rows from database and builds citation map: `{fact_text: {url, title, anchor, confidence}}`. Injected into prompt with **CRITICAL CITATION REQUIREMENTS** (visual emphasis with borders, emojis, explicit formatting). Instructions include: (1) Mandatory inline citations for all factual claims, (2) JSON citation map with verified facts, (3) Format examples for all 3 accepted formats (markdown, parenthetical, footnotes), (4) Minimum citation requirements (0-2 claims = 3 sources, 3+ claims = 1 per claim), (5) **Strict prohibitions** against inventing statistics/percentages/dollar amounts (must use ONLY citation map facts), (6) Proper citation examples (✓ good vs ✗ bad), (7) Visual formatting (╔══╗ borders, ⚠️ warnings, 📋 checklists, 🚫 prohibitions). Citation map typically contains 10-30 facts from 3-5 verified sources. Claude weaves citations naturally into sentences during generation.
...
- **Phase 1.5 Events**: `phase1_5_start` (verification begins), `source_verification` (per-source progress with credibility scores color-coded: green ≥80, yellow ≥60, red <60), `phase1_5_complete` (summary: verified/rejected counts + avg credibility), `error` (generation halted if <3 credible sources)
...
- `DomainCredibilityCache` — 7-day cache for domain authority scores. Stores (`domain` unique, `domain_authority`, `referring_domains`, `is_authoritative`, `is_academic`). Shared across workspaces for efficiency. Reduces DataForSEO API costs by 80%+ after initial warmup.
